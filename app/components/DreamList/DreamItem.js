...
class DreamItem extends Component {
  ...
  componentWillMount() {
    // console.log(this.mapColorsToTags(this.props.dream.tags.length));
    this.setState({colors: this.mapColorsToTags()})
  }
  ...
  deleteIcon = () => {
    if (this.props.isDelete) {
      return (<TouchableWithoutFeedback onPress={() => this.props.onDelete(this.props.dream._id)}>
        <Animated.View style={[
            styles.deleteButtonContainer, {
              opacity: this.state.deleteButtonOpacity,
              transform: [
                {
                  scale: this.state.deleteButtonScale
                }
              ]
            }
          ]}>
          <Image style={{
              ...
              width: '100%',
              height: '100%'
            }} source={require('../../assets/close_button_red.png')}/>
        </Animated.View>
      </TouchableWithoutFeedback>)
    } else {
      return;
  ...
  mapColorsToTags = () => {
    const {tags, length} = this.props.dream
    const colors = Colors(tags.length)
